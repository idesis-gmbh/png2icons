// Original UPNG.js file taken from https://github.com/photopea/UPNG.js

var pako = require("pako");

var UPNG = {
	writeLogMessages: true
};

UPNG.setWriteLogMessages = function(writeLogMessages) {
	UPNG.writeLogMessages = writeLogMessages;
}

UPNG.log = function(...logMessages) {
	if (UPNG.writeLogMessages) {
		console.log("UPNG:", logMessages.join(" "));
	}
}

UPNG.toRGBA8 = function(out)
{
	//UPNG.log(out.ctype, out.depth);
	...
	else UPNG.log("unsupported color type", ctype);
	return bf;
}
...
UPNG.decode = function(buff)
{
	...
	while(true)
	{
		...
		if     (type=="IHDR")  {  UPNG.decode._IHDR(data, offset, out);  }
			...
			else UPNG.log("tRNS for unsupported color type", out.ctype, len);
		}
		...
		else if(type=="bKGD") 
		{
			...
			else UPNG.log("unsupported compression method:", out.interlace);
			
			if(out.filter!=0) UPNG.log("unsupported filter method:", out.filter);
			...
			else UPNG.log("unsupported interlace method:", out.interlace);
			
			break;
		}
		else {  UPNG.log("unknown chunk type", type, len);  }
		offset += len;
...
UPNG.decode._getBPP = function(out) {
	...
	if(noc==null) UPNG.log("unsupported color type", out.ctype);
