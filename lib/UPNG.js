...
var lib = "UPNG"; //path.parse(__filename).name;
var UPNG = {};

UPNG.toRGBA8 = function(out)
{
	//console.log(`${lib}:`, out.ctype, out.depth);
	...
	else console.log(`${lib}: unsupported color type`, ctype);
	return bf;
}
...
UPNG.decode = function(buff)
{
	...
	while(true)
	{
		...
		//console.log(`${lib}:`, offset, len, type);
		...
		if     (type=="IHDR")  {  UPNG.decode._IHDR(data, offset, out);  }
			...
			else console.log(`${lib}: tRNS for unsupported color type`, out.ctype, len);
		}
		...
		else if(type=="bKGD") 
		{
			...
			else console.log(`${lib}: unsupported compression method:`, out.interlace);
			
			if(out.filter!=0) console.log(`${lib}: unsupported filter method:`, out.filter);
			...
			else console.log(`${lib}: unsupported interlace method:`, out.interlace);
			
			break;
		}
		else {  console.log(`${lib}: unknown chunk type`, type, len);  }
		offset += len;
...
UPNG.decode._getBPP = function(out) {
	...
	if(noc==null) console.log(`${lib}: unsupported color type`, out.ctype);
